...
const Icon = L.Icon.extend({
  options: {
    iconUrl: '/assets/project-setup/onstreet/signage.svg',
    iconRetinaUrl: '/assets/project-setup/onstreet/signage.svg',
    iconSize: [48, 48],
    iconAnchor: [12, 41],
    popupAnchor: [1, -34],
    tooltipAnchor: [16, -28],
...
export class SignageComponent extends BaseOnStreetComponent implements OnInit, OnChanges, OnDestroy {
  ...
  currentSignage: Asset = new Asset();
  ...
  ngOnInit() {
    ...
    this.buildForm();
    this.assetModelService.get({type_asset: 'Signage'}).pipe(takeUntil(this.ngUnsubscribe)).subscribe(assetModel => {
      this.assetModels = assetModel;
      this.assetsModelsFiltered = this.assetModels;
    });
    this.getAllItems();
  }
  ...
  private getAllItems() {
    forkJoin(
      this.assetService.get({project_id: this.projectId, type_asset: 'Signage', status: 'Installed'}),
      this.assetService.getAvailable({type_asset : 'Signage'}),
      this.assetService.get({project_id: this.projectId, type_asset: 'Signage'}),
      this.projectService.getProjectById(this.projectId),
      this.projectZoneService.get({project_id: this.projectId}),
    ).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      const [signage, assets, allSignages, project, zone ] = res;
      this.signages = signage;
      this.assetsAvailable = assets.sort((a, b) => a.codification_id > b.codification_id ? 1 : -1);
      this.allAssets = allSignages;
  ...
  public onSiteMapDataChanged(mapdata: any) {
    ...
    if (
      ...
      mapdataCreateObj.features.push(signageMarker);
  ...
  private loadSignageMapData(filter: string) {
    ...
    // add info to a point
    const v: Asset = this.allAssets.filter(signage => signage.codification_id === filter)[0];
    let needToAdd = true;
  ...
  public onChangeName(value: AssetModel) {
    if (!this.currentSignage.codification_id) {
      this.signageName = value.name;
  ...
  private editFormById(id: any) {
    ...
    this.currentSignage = this.signages.find(v => v.codification_id === id);
     if (this.currentSignage && this.currentSignage.codification_id) {
       this.mapPositionCenter = [this.currentSignage.latitude, this.currentSignage.longitude];
       if (this.currentSignage.img_url) {
         this.parkingImages = this.currentSignage.img_url.split(',').map((url) => ((!!url) ? (url.startsWith('uploads') ?  this.apiEndpoint + '/' + url : url) : ''));
         this.selectedImg = {
      ...
      const assetModel = this.assetModels.find( asstModel => asstModel.code === this.currentSignage.model_code);
      if (assetModel) {
      ...
      asset.codification_id = this.currentSignage.codification_id;
      this.assetsAvailableFiltered = [asset];
      this.form.reset({
        name: this.currentSignage.model_txt,
        latitude: this.currentSignage.latitude,
        longitude: this.currentSignage.longitude,
        codificationId: this.currentSignage.codification_id,
      ...
      const mapdataObj = {
       ...
       const icon = this.currentSignage.img_url
         ? this.currentSignage.img_url.startsWith('uploads')
           ? this.apiEndpoint + '/' + this.currentSignage.img_url
           : this.currentSignage.img_url
         : this.currentSignage.model_img_url || 'No Icon';
      ...
      const signageMarker = {
        ...
        geometry: {
          type: 'Point',
          coordinates: [this.currentSignage.latitude, this.currentSignage.longitude]
        }
      };
  ...
  public onSubmit() {
    if (this.formValid(this.form, [''])) {
      ...
      // const modelChange = this.assetModels.find(model => model.name === form.name);
      // modelChange.fullspecs_link = form.fullspecs_link;
      this.assetService.update(asset)
        .pipe(takeUntil(this.ngUnsubscribe))
        .subscribe(res => {
          if (this.currentSignage.codification_id) {
            this.toastr.success('Signage is updated successfully!', 'Success!');
          } else {
            this.toastr.success('Signage is created successfully!', 'Success!');
          }
          this.resetForm();
        }, err => {
